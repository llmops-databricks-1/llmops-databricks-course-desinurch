...
# Update values to match your currently negotiated/public pricing
...
normalized_df = (
    clean_df
    .withColumn("model_lower", F.lower(F.col("data_model")))
    .withColumn(
        "provider",
        F.when(
        ...
        .when(
            F.col("model_lower").rlike("bedrock|anthropic|claude|llama|mistral|cohere|ai21|amazon\\."),
            F.lit("bedrock"),
        )
...
joined_df = (
    ...
    .join(
        ...
        (
            ...
            & (
                F.expr(
                    "l.model_lower LIKE concat('%', lower(p.model_match), '%')"
                )
            )
        ),
        "left",
...
(
    joined_df
    .filter(F.col("input_usd_per_1m").isNull() | F.col("output_usd_per_1m").isNull())
